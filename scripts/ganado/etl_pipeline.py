...
traz = pd.read_csv('/sessions/zen-hopeful-newton/mnt/uploads/TRAZABILIDAD.csv')
guias = pd.read_excel('/sessions/zen-hopeful-newton/mnt/uploads/GUIAS ACTUALIZADAS.xlsx')
compras = pd.read_csv('/sessions/zen-hopeful-newton/mnt/uploads/COMPRAS DE DESMAMANTES - BD COMPRAS.csv')
...
def log_clean(rule, affected, detail=""):
    cleaning_log.append({"rule": rule, "affected_rows": affected, "detail": detail})
...
compras['PROPRIETARIO'] = compras['PROPRIETARIO'].map(lambda x: propietario_map.get(str(x).strip(), str(x).strip()))
log_clean("Standardize PROPRIETARIO", before - compras['PROPRIETARIO'].nunique(), f"now {compras['PROPRIETARIO'].nunique()} unique")
...
for val in compras['ESTABLECIMENTO DE DESTINO'].unique():
    ...
    if clean == 'LUISPAR': clean = 'LUSIPAR'
    dest_map[val] = clean
...
compras['PROVEEDOR'] = compras['PROVEEDOR'].apply(lambda x: str(x).strip().upper() if pd.notna(x) else x)
compras['ESTABLECIMENTO ORIGEN'] = compras['ESTABLECIMENTO ORIGEN'].apply(lambda x: str(x).strip().upper() if pd.notna(x) else x)
compras['INTERMEDIARIO'] = compras['INTERMEDIARIO'].apply(lambda x: str(x).strip().upper() if pd.notna(x) else x)
compras['FLETERO'] = compras['FLETERO'].apply(lambda x: str(x).strip().upper() if pd.notna(x) else x)
...
fazendas_origen = guias_clean[['nombre_establecimiento_origen', 'latitud_origen', 'longitud_origen']].dropna(
    subset=['nombre_establecimiento_origen']).rename(columns={
    'nombre_establecimiento_origen': 'nome',
    'latitud_origen': 'latitude',
    'longitud_origen': 'longitude'
})

fazendas_destino = guias_clean[['nombre_establecimiento_destino', 'latitud_destino', 'longitud_destino']].dropna(
    subset=['nombre_establecimiento_destino']).rename(columns={
...
compras_origins = set(compras_deduped['ESTABLECIMENTO ORIGEN'].dropna().unique())
all_estabs = compras_estabs | compras_origins
guias_estabs = set(fazendas_geo['nome'].unique())
...
print(f"  FAZENDAS: {len(df_fazendas)} total ({(df_fazendas['tipo']=='PROPRIA').sum()} own, {(df_fazendas['tipo']=='PROVEDOR').sum()} supplier)")
...
df_compras = compras_deduped[['id_compra', 'FECHA_PARSED', 'PROVEEDOR', 'ESTABLECIMENTO ORIGEN',
                               ...
                               'PAIS', 'ANO', 'NRO. FACTURA DE COMPRA', 'SOLA MARCA']].copy()
...
df_guias = guias_clean[['id_guia_traslado', 'nro_guia', 'nro_cota', 'fecha_emision', 'finalidad',
                         ...
                         'codigo_categoria', 'descripcion_categoria', 'cantidad_animales']].copy()
...
def build_geojson(df, role):
    ...
    for _, row in df.iterrows():
        if pd.isna(row['latitude']) or pd.isna(row['longitude']):
            continue
        props = {k: (v if pd.notna(v) and not isinstance(v, float) else
...
top_prov_vol = prov_stats.nlargest(20, 'total_animales')[['nome', 'n_compras', 'total_animales', 'primera_compra', 'ultima_compra']]
analytics['top_provedores_volume'] = top_prov_vol.to_dict('records')
print(f"  Top 20 provedores by volume computed")
...
out_dir = '/sessions/zen-hopeful-newton/ganado_pipeline/data'

# Entity tables
df_fazendas.to_csv(f'{out_dir}/fazendas.csv', index=False)
prov_stats.to_csv(f'{out_dir}/provedores.csv', index=False)
df_compras.to_csv(f'{out_dir}/compras.csv', index=False)
df_guias.to_csv(f'{out_dir}/guias.csv', index=False)
df_movs.to_csv(f'{out_dir}/movimentacoes.csv', index=False)

# Consolidated base
consolidated = compras_deduped.copy()
consolidated.to_csv(f'{out_dir}/base_consolidada_compras.csv', index=False)

# Geo data
geo_origins.to_csv(f'{out_dir}/geo_fazendas_origen.csv', index=False)
geo_destinos.to_csv(f'{out_dir}/geo_fazendas_destino.csv', index=False)

with open('/sessions/zen-hopeful-newton/ganado_pipeline/geo/fazendas.geojson', 'w') as f:
