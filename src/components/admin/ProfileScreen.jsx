...
export default function ProfileScreen({ onBack, currentUser }) {
  ...
  return (
    <div className="pb-[120px] md:pb-8 animate-fade-in">
      ...
      <div className="px-5 flex flex-col items-center pt-4">
        {/* Avatar */}
        <div className="w-20 h-20 rounded-full bg-[#1F2A44] flex items-center justify-center text-2xl font-bold text-[#C8A03A] mb-3 border-2 border-[#C8A03A]/20">
          {initials}
        </div>
        <h2 className="text-xl font-bold text-white m-0">{name}</h2>
        <span className="text-xs font-semibold text-[#C8A03A] bg-[#C8A03A]/10 px-3 py-1 rounded-full mt-1.5">
          {roleLabel}
        </span>
      </div>
