  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && filtered.length > 0 && (
        <div className="absolute z-50 left-0 right-0 mt-1 bg-[var(--color-modal)] border border-[var(--color-border)] rounded-lg shadow-xl max-h-[200px] overflow-y-auto">
          {filtered.map(n => (
            <button
              key={n}
...
export default function UserFormModal({ user, title, establishments, existingNames, loading, onSave, onResetPassword, onClose }) {
  ...
  return (
    ...
    >
      <div
        className="bg-[var(--color-modal)] rounded-2xl p-6 w-full max-w-[440px] max-h-[90vh] overflow-auto shadow-2xl border border-[var(--color-border)]"
