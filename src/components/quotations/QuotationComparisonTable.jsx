      ...
      <table className="w-full text-xs border-collapse">
        <thead>
          <tr className="border-b border-white/[0.06]">
            <th className="text-left px-3 py-2.5 text-slate-400 font-semibold sticky left-0 bg-[#0B1120] min-w-[100px]">Item</th>
            {quotations.map(q => (
        ...
        <tbody>
          {items.map((it, idx) => (
            <tr key={idx} className="border-b border-white/[0.06]">
              <td className="px-3 py-2 text-white sticky left-0 bg-[#0B1120]">
          ...
          <tr className="bg-white/[0.02]">
            <td className="px-3 py-2.5 text-white font-bold sticky left-0 bg-[#0B1120]">TOTAL</td>
            {quotations.map(q => {
          ...
          {/* Delivery row */}
          <tr>
            <td className="px-3 py-2 text-slate-400 sticky left-0 bg-[#0B1120]">Entrega</td>
            {quotations.map(q => {
