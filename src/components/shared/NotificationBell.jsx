  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        className="w-9 h-9 rounded-lg bg-[var(--color-surface)] border-none cursor-pointer flex items-center justify-center text-base hover:bg-[var(--color-surface-hover)] transition-colors relative"
      ...
      {open && (
        <div className="absolute left-0 top-full mt-2 w-[340px] max-w-[calc(100vw-16px)] bg-[var(--color-modal)] border border-[var(--color-border)] rounded-xl shadow-2xl z-[200] overflow-hidden">
          {/* Header */}
          <div className="px-4 py-3 border-b border-[var(--color-border)] flex justify-between items-center">
            ...
            {unreadCount > 0 && (
              <button
                ...
                className="text-[11px] text-[#C8A03A] font-medium bg-transparent border-none cursor-pointer hover:text-[#C8A03A]"
              >
            ...
            {visible.length === 0 ? (
              ...
              visible.slice(0, 20).map(notif => (
                <button
                  ...
                  className={`w-full text-left px-4 py-3 border-b border-[var(--color-border)] cursor-pointer transition-colors flex gap-3 items-start ${
                    notif.read ? 'bg-transparent hover:bg-[var(--color-surface)]' : 'bg-[var(--color-surface)] hover:bg-[var(--color-surface-hover)]'
                  } border-none`}
                >
                  {/* Unread dot */}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className={`text-xs leading-relaxed line-clamp-2 ${notif.read ? 'text-slate-400' : 'text-[var(--color-text)] font-medium'}`}>
                      {notif.message}
                    </div>
